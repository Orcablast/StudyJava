...
public class Example05 {
	...
	public void exam01() { // 주석 버전은 아래 메소드 참고
		...
		if (num % 2 == 1) { // 입력한 변수 num이 홀수일 때
			...
			for (int i = 0; i < arr.length; i++) {
				...
				if (i < (num / 2 + 1)) {
					...
					// 예시) i가 0이라면 - > arr[0] = 0+1;
					// 예시) i가 2 라면 - > arr[2] = 2+1;
					arr[i] = i + 1;

					// i값이 num/2 + 1 값보다 크거나 같은 경우
					// num이 7이라면
					// -> 7/2 +1 보다 크거나 같을 경우 -> i >= 4
					// i값이 4, 5, 6일때 true
				} else if (i >= (num / 2 + 1)) {
					
					// arr배열 i번째 값 i번째 [i-1]번째 값  
					// 예시) i가 4라면 -> arr[4] = arr[3] -1
					// 예시) i가 6이라면 -> arr[6] = arr[5] -1
					arr[i] = arr[i - 1] - 1;
				}
				
				System.out.print(arr[i]+" ");
			}						
		} else { // 입력받은 값이 홀수가 아닐때
			System.out.println("잘못 입력하셨습니다.");
		}
	}
}