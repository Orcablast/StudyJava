...
public class SearchWord {
	...
	BufferedReader br; // 테스트용 객체 생성, 삭제되어야함
	BufferedWriter bw; // 테스트용 객체 생성, 삭제되어야함
	BufferedReader failbr; // 테스트용 객체 생성, 삭제되어야함
	...
	public SearchWord() { // 단어장 전체 읽어와 리스트에 추가
		...
		try {
			...
			FileReader fr = new FileReader("failDB.txt"); // 오답파일 불러옴
			failbr = new BufferedReader(fr);
			FileReader fr2 = new FileReader("failDB.txt"); // 오답파일 불러옴
			failbr2 = new BufferedReader(fr2);
			while (true) {
				String failstr = failbr.readLine(); // 오답파일에서 텍스트파일 1줄씩 읽어옴
				String toke = failbr2.readLine();

				if (failstr == null || toke == null) { // 더이상 불러올 텍스트 없을 때
					break;
				}
				...
				StringTokenizer sT = new StringTokenizer(failstr, "/"); // 오답 한줄씩 토큰으로 나눠주기
	...
	public void testMenu() {
		...
		try {
			FileReader fr2 = new FileReader("failDB.txt"); // 오답노트 읽어옴
			...
			while (true) {
				...
				switch (select) {
				case 1: // 영한테스트라면
					ArrayList<Word> failList = view.engTest(allDB); // 모든리스트를 주고 틀린리스트를 받는다.
					if (failList != null) { // 받은리스트에 정보가 있다면
				...
				case 2: // 한영테스트라면
					...
					if (failList2 != null) { // 받은리스트에 정보가 있다면
	...
	public void output() {
		...
		try {
			FileWriter fw = new FileWriter("failDB.txt"); // 오답노트 생성
	...
	public void reTest() { // 재시험 보고 정답 시 오답노트에서 삭제시키기
		...
		try {
			FileReader fr = new FileReader("failDB.txt"); // 오답파일 불러오기
	...
	public void clearNote() {
		...
		try {
			FileReader fr = new FileReader("failDB.txt"); // 오답전체파일 불러오기
			br = new BufferedReader(fr);
			while (true) {
	...
	public void readNote() { // 오답노트 넘버링 보고 선택해서 뜻까지 조회
		...
		if (failName.size() == 0) { // 보여줄 오답이 없을 시
			...
			System.out.println("단어 : " + failDB.get(select - 1).getName() + " 첫번째 뜻 : "
					+ failDB.get(select - 1).getMean1() + " 두번째 뜻 : " + failDB.get(select - 1).getMean2()); // 뜻 포함한 영단어
																											// 들어있는 배열에서
																											// 선택한 오답 출력
			System.out.println();
		}
	...
	public void store() {
		BufferedWriter bw=null; 
		try {
			FileWriter fw=new FileWriter("allDB.txt"); //파일 쓰기(등록한 단어 사용하려고)
			bw=new BufferedWriter(fw);
